...
// @id             panda-yadg
// @name           panda.cd - YADG
// @description    This script provides integration with online description generator YADG (http://yadg.cc) Based on userscript by Slack06@github (v. 1.3.4).
// @license        https://github.com/Slack06/yadg-userscript/blob/master/LICENSE
// @version        0.9
// @namespace      yadg
// @grant          GM_xmlhttpRequest
// @updateURL      https://raw.githubusercontent.com/knutshub/panda-yadg/master/pandacd_yadg.user.js
// @downloadURL    https://raw.githubusercontent.com/knutshub/panda-yadg/master/pandacd_yadg.user.js
// @require        https://yadg.cc/static/js/jsandbox.min.js
// @include        http*://*panda.cd/upload/p__action__showform*
// @include        http*://*panda.cd/index.php?p=requests*
// @include        http*://*panda.cd/torrents/*
// ==/UserScript==
...
var factory = {
    ...
    locations : new Array(
        {
            name : 'panda_upload',
            regex : /http(s)?\:\/\/(.*\.)?panda\.cd\/upload\/p__action__showform*/i
        },
        // TODO: edit this!
        //{
//            name : 'panda_edit',
...
//        },
        {
            name : 'panda_request',
            regex : /http(s)?\:\/\/(.*\.)?panda\.cd\/index\.php\?p=requests&action=new/i
        },
        {
            name : 'panda_request_edit',
            regex : /http(s)?\:\/\/(.*\.)?panda\.cd\/index\.php\?p=requests&action=edit&id=.*/i
        },
        {
            name : 'panda_torrent_overview',
            regex : /http(s)?\:\/\/(.*\.)?panda\.cd\/torrents\/.*/i
        }
    ...
    setDefaultFormat : function() {
        ...
        if (default_format !== null && default_format in format_offsets) {
            ...
            switch (this.currentLocation) {
                default:
                    format_select.selectedIndex = format_offsets[defaultPandaFormat];
                    break;
            }
        }
    },
    ...
    getInputElements : function() {
        ...
        switch (this.currentLocation) {
            ...
            case "panda_upload":
            case "panda_request":
            case "panda_request_edit":
                var tr = document.createElement('tr');
                tr.className = "yadg_tr";
                tr.innerHTML = '<td class="rowa">YADG:</td><td class="rowb">' + inputHTML + scraperSelectHTML + scraperInfoLink + buttonHTML + toggleOptionsLinkHTML + optionsHTML + responseDivHTML + '</td>';
                return tr;
    ...
    insertIntoPage : function(element) {
        switch (this.currentLocation) {
            ...
            case "panda_upload":
                var year_tr = document.getElementById('year');
                year_tr.parentNode.parentNode.parentNode.insertBefore(element,year_tr.parentNode.parentNode);
                break;

            case "panda_request":
            case "panda_request_edit":
                var artist_tr = document.getElementById('artistfields');
                artist_tr.parentNode.parentNode.insertBefore(element,artist_tr.parentNode);
                break;
    ...
    getDescriptionBox : function() {
        switch (this.currentLocation) {
            ...
            case "panda_upload":
                return document.getElementById('album_descr');
            ...
            case "panda_upload":
                var f = function(rawData) {
                    var artist_inputs = document.getElementsByName("artists[]"),
                        album_title_input = document.getElementById("album"),
